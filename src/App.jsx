...
function App() {
  ...
  const checkSolution = async () => {
    ...
    try {
      const res = await puter.ai.chat(
        `
          You are helpful interview coach. Given the interview question below, evaluate the provided solution code. 
          Provide feedback on its correctness, efficiency, and style. 
          If correct, respond with "✅ Correct! Well done." only. If incorrect, provide constructive feedback/hints on what is wrong and how to improve it, but don't reveal the full answer.
  ...
  return (
   <main className='min-h-screen bg-gradient-to-br from-sky-900 via-slate-900 to-emerald-900 flex flex-col items-center justify-center p-6 gap-10'>
    ...
    {!questionData ? (
      ...
      <div className='w-full max-w-md p-10 bg-gray-900/80 backdrop-blur-md border border-gray-700 rounded-3xl shadow-lg shadow-sky-600 hover:shadow-sky-300 hover:shadow-2xl transition duration-300 text-center'>
      ...
      <div className='space-y-6 w-full'>
        <div className='grid lg:grid-cols-2 gap-6'>
          <div className='bg-gradient-to-br from-blue-950/40 to bg-sky-950/50 backdrop-blur-sm border border-indigo-400/30 rounded-2xl shadow-2xl p-8 space-y-4'>
            <div>
        ...
        <div className='flex gap-6 lg:gap-10 justify-center items-center flex-col lg:flex-row'>
          ...
          <div className='flex gap-3 items-center flex-wrap'>
              ...
              {["Beginner", "Medium", "Intermediate"].map((level) => (
              <button key={level} onClick={() => handleDifficultySelect(level)} className={`px-4 py-2 rounded-full font-semibold transition-colors duration-200 cursor-pointer ${difficulty === level ? "bg-blue-500 text-white shadow-md" : "bg-gray-700 text-gray-300 hover:bg-gray-600"}`}>{level}</button>
            ))}
          </div>
        </div>

        {feedback && (
          <div className={`rounded-3xl p-6 shadow-2xl backdrop-blur-sm ${feedback.includes("✅") ? "bg-green-900/40 border border-green-500/30" : feedback.includes("❌") ? "bg-red-900/40 border border-red-500/30" : "bg-gray-800/30 border border-gray-700/50"}`}>
